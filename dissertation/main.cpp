...
//Transfer data dense matrix A to dA
   ...
   cudaMemcpy(d_r, rhs, sizeof(float) * M, cudaMemcpyHostToDevice);
   cudaMemcpy(dA, A, sizeof(float) * M * N, cudaMemcpyHostToDevice);

// Compute the number of non-zero elements in A
    cusparseSnnz(cusparseHandle, CUSPARSE_DIRECTION_ROW, M, N, descr, dA, M, dNnzPerRow, &totalNnz);
//this part check if we have correct results from the last function cusparseSnnz
    ...
    cudaMalloc((void **)&d_val, sizeof(float) * totalNnz);
    cudaMalloc((void **)&d_row, sizeof(int) * (M + 1));
    cudaMalloc((void **)&d_col, sizeof(int) * totalNnz);
 ...
 // Convert A from a dense formatting to a CSR formatting, using the GPU
   cusparseSdense2csr(cusparseHandle, M, N, descr, dA, M, dNnzPerRow,
                                      d_val, d_row, d_col);
