...
//implement matrix Aglobal to csr
 
   
   A = (float *)malloc(sizeof(float)*((glodim+1)*(glodim+1)));
   ...
   for(int i=0;i<glodim;i++)
       {
        ...
        for(int j=0;j<glodim;j++)
            {
               A[(glodim)*j+i]=Aglobal[i][j];
               printf("A[%d]=%f \n",(glodim)*j+i,A[(glodim)*j+i]);
            }
       }
...
*/
    ...
    CHECK_CUSPARSE(cusparseSnnz(cusparseHandle, CUSPARSE_DIRECTION_ROW, M, N, descr, dA,
...
// Convert A from a dense formatting to a CSR formatting, using the GPU
 //   CHECK_CUSPARSE(cusparseSdense2csr(cusparseHandle, M, N, descr, dA, M, dNnzPerRow,d_val, d_row, d_col));
