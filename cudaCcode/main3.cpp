...
{
    int M = 0, N = 0, nz = 0,nnzb=0, *I = NULL, *J = NULL;
    int *Ib = NULL, *Jb = NULL;
    float *val = NULL;
    double *valb = NULL;
...
//allocating values from  Aglobal
    ...
    Jb = (int *)malloc(sizeof(int)*nz);
    valb = (double *)malloc(sizeof(double)*nnzb);
...
//allocating memory for matrix in csr from for Aglobal
   ...
   checkCudaErrors(cudaMalloc((void **)&d_rowb, (glodim+1)*sizeof(int)));
    ...
    cusparseDdense2csr(cusparseHandle, glodim,glodim,descr,d_Bglobal,glodim,nnzPerRowColumn,d_valb,d_rowb,d_colb);
