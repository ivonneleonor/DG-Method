...
//implement matrix Aglobal to csr line 443
   ...
   double *Bglobal = NULL;
   Bglobal = (double *)malloc(sizeof(double)*(glodim*glodim));
...
for(int i=0;i<glodim;i++)
       {
        ...
        for(int j=0;j<glodim;j++)
            {
               Bglobal[glodim*i+j]=Aglobal[i][j];
               printf("Bglobal[%d]=%f \n",glodim*i+j,Bglobal[glodim*i+j]);
            }
       }
...
//cusparseDnnz(cusparseHandle,CUSPARSE_DIRECTION_ROW,glodim,glodim,descr,Aglobal,glodim, &nnzPerRowColumn, &nnzTotalDevHostPtr)
